...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	r.ParseMultipartForm(20 << 20) // 10 MB limit
	...
	if err == nil {
		...
		if handler.Size > 20<<20 { // 20 MB
			data := map[string]interface{}{
				"Error": "Dosya boyutu 20 MB'ı aşmamalıdır.",
			}
			utils.RenderTemplate(w, "templates/uploadForm.html", data)
			return
		}
		//Dosya uzantısını kontrol et
		ext := strings.ToLower(filepath.Ext(handler.Filename))
		if ext != ".jpg" && ext != ".jpeg" && ext != ".png" && ext != ".gif" {
			// Desteklenmeyen dosya türü için hata mesajı göster
			utils.RenderTemplate(w, "templates/uploadForm.html", map[string]interface{}{
				"ErrorMessage": "Unsupported file type. Please upload JPEG, PNG, or GIF.",
			})
			return
		}
		...
		filePath = "static/images/default-image.png.jpg"
	}
